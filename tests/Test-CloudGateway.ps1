...
if ($null -ne $Readme) {
    ...
    Assert-Contains $Readme './cli-proxy-api/cli-proxy-api -config ./config.yaml -codex-device-login' "default README documents Linux device login"
...
if ($null -ne $EnglishReadme) {
    ...
    Assert-Contains $EnglishReadme './cli-proxy-api/cli-proxy-api -config ./config.yaml -codex-device-login' "English README documents Linux device login"
...
if ((Test-Path -LiteralPath $PowerShellGenerator) -and (Test-Path -LiteralPath $PowerShellLanStarter)) {
    ...
    try {
        ...
        Assert-Contains $LanStarterSource "Test-UpstreamProxyEndpoint" "PowerShell LAN starter preflights upstream proxy endpoints"
        Assert-Contains $LanStarterSource "TcpTestSucceeded" "PowerShell LAN starter detects unreachable upstream proxy ports"
        Assert-Contains $LanStarterSource "Get-FreeTcpPort" "PowerShell LAN starter avoids occupied CLIProxyAPI ports"
        Assert-Contains $LanStarterSource "LAN starter will use free private port" "PowerShell LAN starter reports fallback private ports"
